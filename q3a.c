...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        printf("Usage: %s <input_image_path> <output_image_path1> <output_image_path2>\n", argv[0]);
        return 1;
    }

    // Assign command line arguments to IN, OUT and OUT2
    IN = argv[1];
    OUT = argv[2];
    OUT2 = argv[3];
        ...
        for (col = 0; col < M; col++) {
            ...
            for (rowOffset = -size; rowOffset <= size; rowOffset++) {
                for (colOffset = -size; colOffset <= size; colOffset++) {
                    ...
                    if ((row + rowOffset < 0) || (row + rowOffset >= N) || (col + colOffset < 0) ||
                        (col + colOffset >= M))
                        pix = 0;
                    else
